...
contract DEX {
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        uint256 ethReserve = address(this).balance - msg.value; // pre-trade reserve
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        require(msg.value > 0, "deposit(): must include eth");
        uint256 ethReserve = address(this).balance - msg.value; // pre-deposit reserve
        uint256 tokenReserve = token.balanceOf(address(this));
        tokensDeposited = (msg.value * tokenReserve) / ethReserve + 1;
        require(token.balanceOf(msg.sender) >= tokensDeposited, "deposit(): insufficient token balance");
        require(token.allowance(msg.sender, address(this)) >= tokensDeposited, "deposit(): insufficient allowance");

        uint256 liquidityMinted = (msg.value * totalLiquidity) / ethReserve;
        liquidity[msg.sender] += liquidityMinted;
        totalLiquidity += liquidityMinted;

        require(token.transferFrom(msg.sender, address(this), tokensDeposited), "deposit(): token transfer failed");
